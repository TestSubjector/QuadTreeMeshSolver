        ...
        if idx > 0:
            ...
            if flag == 1:
                if xposf == 2:
                    ...
                    globaldata = fixXpos(idx,globaldata,nbhs,-1,30,False,polygonData)
                elif xposf == 1:
                    nbhs = getNeighboursFromTriangle(idx,globaldata,polygonData)
                    nbhs = getDXPosPointsFromSetRaw(idx,globaldata,nbhs)       
                    nbhs = getAeroPointsFromSet(idx,nbhs,globaldata,wallpoints)
                    globaldata = fixXpos(idx,globaldata,nbhs,-2,30,True,polygonData)
                if xnegf == 2:
                    nbhs = getNeighboursFromTriangle(idx,globaldata,polygonData)
                    nbhs = getDXNegPointsFromSetRaw(idx,globaldata,nbhs)   
                    nbhs = getAeroPointsFromSet(idx,nbhs,globaldata,wallpoints)
                    globaldata = fixXneg(idx,globaldata,nbhs,-1,30,False,polygonData)
                elif xnegf == 1:
                    nbhs = getNeighboursFromTriangle(idx,globaldata,polygonData)
                    nbhs = getDXNegPointsFromSetRaw(idx,globaldata,nbhs)       
                    nbhs = getAeroPointsFromSet(idx,nbhs,globaldata,wallpoints)
                    globaldata = fixXneg(idx,globaldata,nbhs,-2,30,True,polygonData)
                if yposf == 2:
                    nbhs = getNeighboursFromTriangle(idx,globaldata,polygonData)
                    nbhs = getDYPosPointsFromSetRaw(idx,globaldata,nbhs)   
                    nbhs = getAeroPointsFromSet(idx,nbhs,globaldata,wallpoints)
                    globaldata = fixYpos(idx,globaldata,nbhs,-1,30,False,polygonData)
                elif yposf == 1:
                    nbhs = getNeighboursFromTriangle(idx,globaldata,polygonData)
                    nbhs = getDYPosPointsFromSetRaw(idx,globaldata,nbhs)       
                    nbhs = getAeroPointsFromSet(idx,nbhs,globaldata,wallpoints)
                    globaldata = fixYpos(idx,globaldata,nbhs,-2,30,True,polygonData)
                if ynegf == 2:
                    nbhs = getNeighboursFromTriangle(idx,globaldata,polygonData)
                    nbhs = getDYNegPointsFromSetRaw(idx,globaldata,nbhs)   
                    nbhs = getAeroPointsFromSet(idx,nbhs,globaldata,wallpoints)
                    globaldata = fixYneg(idx,globaldata,nbhs,-1,30,False,polygonData)
                elif ynegf == 1:
                    nbhs = getNeighboursFromTriangle(idx,globaldata,polygonData)
                    nbhs = getDYNegPointsFromSetRaw(idx,globaldata,nbhs)       
                    nbhs = getAeroPointsFromSet(idx,nbhs,globaldata,wallpoints)
                    globaldata = fixYneg(idx,globaldata,nbhs,-2,30,True,polygonData)
...
def fixXpos(idx,globaldata,nbhs,control,conditionNumber,aggressive,polygonData):
    if control > 0:
        return globaldata
    else:
        control = control + 1
        ...
        if len(conditionSet) > 0:
            ...
            fixXpos(idx,globaldata,nbhs,control,conditionNumber,False,polygonData)
        else:
            if aggressive == True:
                ...
                control = control - 1
                fixXpos(idx,globaldata,nbhofnbh,control,conditionNumber,False,polygonData)
            else:
                return globaldata
    return globaldata

def fixXneg(idx,globaldata,nbhs,control,conditionNumber,aggressive,polygonData):
    if control > 0:
        return globaldata
    else:
        control = control + 1
        ...
        if len(conditionSet) > 0:
            conditionSet.sort(key=lambda x: x[1])
            globaldata = appendNeighbours(idx, globaldata, conditionSet[0][0])
            fixXneg(idx,globaldata,nbhs,control,conditionNumber,False,polygonData)
        else:
            if aggressive == True:
                directnbhs = getNeighboursFromTriangle(idx,globaldata,polygonData)
                nbhofnbh = []
                ...
                nbhofnbh = list(set(nbhofnbh))
                control = control - 1
                fixXneg(idx,globaldata,nbhofnbh,control,conditionNumber,False,polygonData)
            else:
                return globaldata
    return globaldata
        
def fixYpos(idx,globaldata,nbhs,control,conditionNumber,aggressive,polygonData):
    if control > 0:
        return globaldata
    else:
        control = control + 1
        ...
        if len(conditionSet) > 0:
            conditionSet.sort(key=lambda x: x[1])
            globaldata = appendNeighbours(idx, globaldata, conditionSet[0][0])
            fixYpos(idx,globaldata,nbhs,control,conditionNumber,False,polygonData)
        else:
            if aggressive == True:
                directnbhs = getNeighboursFromTriangle(idx,globaldata,polygonData)
                nbhofnbh = []
                ...
                nbhofnbh = list(set(nbhofnbh))
                control = control - 1
                fixYpos(idx,globaldata,nbhofnbh,control,conditionNumber,False,polygonData)
            else:
                return globaldata
    return globaldata
...
def fixYneg(idx,globaldata,nbhs,control,conditionNumber,aggressive,polygonData):
    if control > 0:
        ...
        if len(conditionSet) > 0:
            conditionSet.sort(key=lambda x: x[1])
            globaldata = appendNeighbours(idx, globaldata, conditionSet[0][0])
            fixYneg(idx,globaldata,nbhs,control,conditionNumber,False,polygonData)
        else:
            if aggressive == True:
                directnbhs = getNeighboursFromTriangle(idx,globaldata,polygonData)
                nbhofnbh = []
                ...
                nbhofnbh = list(set(nbhofnbh))
                control = control - 1
                fixYneg(idx,globaldata,nbhofnbh,control,conditionNumber,False,polygonData)
            else:
                return globaldata
    return globaldata