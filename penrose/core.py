...
def getXPosPenrose(index,globaldata,threshold,power):
    try:
        return getMoorePenroseMatrix(index,getDXPosPoints(index,globaldata),globaldata,threshold,power)
    except:
        return np.array(["Only 1 Point. Couldn't get MP Inverse"])

def getXNegPenrose(index,globaldata,threshold,power):
    try:
        return getMoorePenroseMatrix(index,getDXNegPoints(index,globaldata),globaldata,threshold,power)
    except:
        return np.array(["Only 1 Point. Couldn't get MP Inverse"])

def getYPosPenrose(index,globaldata,threshold,power):
    try:
        return getMoorePenroseMatrix(index,getDYPosPoints(index,globaldata),globaldata,threshold,power)
    except:
        return np.array(["Only 1 Point. Couldn't get MP Inverse"])

def getYNegPenrose(index,globaldata,threshold,power):
    try:
        return getMoorePenroseMatrix(index,getDYNegPoints(index,globaldata),globaldata,threshold,power)
    except:
        return np.array(["Only 1 Point. Couldn't get MP Inverse"])
