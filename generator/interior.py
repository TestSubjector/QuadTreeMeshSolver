...
def conditionValueFixForXPos(
    ...
    if initialConditionValue > threshold:
        ...
        if len(newdxpospoints) == 0:
            writeLog(
                [
                    "We tried finding neighbour of neighbours but none satisfy the delta xpos condition for",
                    getPoint(index, globaldata),
                ]
            )
        else:
            finalList = []
            ...
            if len(finalList) == 0 or finalList is None:
                writeLog(
                    [
                        "We tried finding points to reduce threshold value to",
                        threshold,
                        "but couldn't find any for index",
                        index,
                    ]
                )
...
def conditionValueFixForXNeg(
    ...
    if initialConditionValue > threshold:
        ...
        if len(newdxpospoints) == 0:
            writeLog(
                [
                    "We tried finding neighbour of neighbours but none satisfy the delta xneg condition for",
                    getPoint(index, globaldata),
                ]
            )
        else:
...
def conditionValueFixForYPos(
    ...
    if initialConditionValue > threshold:
        ...
        if len(newdxpospoints) == 0:
            writeLog(
                [
                    "We tried finding neighbour of neighbours but none satisfy the delta ypos condition for",
                    getPoint(index, globaldata),
                ]
            )
        else:
...
def conditionValueFixForYNeg(
    ...
    if initialConditionValue > threshold:
        ...
        if len(newdxpospoints) == 0:
            writeLog(
                [
                    "We tried finding neighbour of neighbours but none satisfy the delta yneg condition for",
                    getPoint(index, globaldata),
                ]
            )
        else:
...
def printPosDeltaConditions(index, globaldata, hashtable, threshold):
    ...
    if (
        initialConditionValueXNeg > threshold
        or initialConditionValueXPos > threshold
        or initialConditionValueYPos > threshold
        or initialConditionValueYNeg > threshold
    ):
        # print(index,len(dSPointXPos),initialConditionValueXPos,len(dSPointXNeg),initialConditionValueXNeg,len(dSPointYPos),initialConditionValueYPos,len(dSPointYNeg),initialConditionValueYNeg)
        writeLog(
            [
                index,
                len(dSPointXPos),
                initialConditionValueXPos,
                len(dSPointXNeg),
                initialConditionValueXNeg,
                len(dSPointYPos),
                initialConditionValueYPos,
                len(dSPointYNeg),
                initialConditionValueYNeg,
            ]
        )
        printPosDeltaConditions2(index, globaldata, hashtable)
...
def printPosDeltaPointConditions(index, globaldata, hashtable, threshold):
    ...
    dSPointYNeg = getDYNegPoints(index, globaldata, hashtable)
    if (
        initialConditionValueXPos > threshold
        or initialConditionValueXNeg > threshold
        or initialConditionValueYPos > threshold
        or initialConditionValueYNeg > threshold
    ):
        print(
...
def setPosDeltaFlags(index, globaldata, hashtable, threshold, problempts, flag):
    ...
    if (
        initialConditionValueXPos > threshold
        or math.isnan(initialConditionValueXPos)
        or math.isinf(initialConditionValueXPos)
    ):
    ...
    if (
        initialConditionValueXNeg > threshold
        or math.isnan(initialConditionValueXNeg)
        or math.isinf(initialConditionValueXNeg)
    ):
        problempts.append(index)
        # globaldata = setFlagValue(index,8,1,globaldata)
        # writeLog([index,"Full Stencil Condition Value",conditionValueOfPointFull(index,globaldata)])
    if (
        initialConditionValueYPos > threshold
        or math.isnan(initialConditionValueYPos)
        or math.isinf(initialConditionValueYPos)
    ):
        problempts.append(index)
        # globaldata = setFlagValue(index,9,1,globaldata)
        # writeLog([index,"Full Stencil Condition Value",conditionValueOfPointFull(index,globaldata)])
    if (
        initialConditionValueYNeg > threshold
        or math.isnan(initialConditionValueYNeg)
        or math.isinf(initialConditionValueYNeg)
    ):
        problempts.append(index)
    ...
    for wallpointset in wallpoints:
        for idx, _ in enumerate(wallpointset):
            ...
            if distance <= threshold:
                ...
                nbhpt = getPoint(
                    getIndexOf(wallpointset[closestwallpointnbh], hashtable), globaldata
                )
                for index, individualitem in enumerate(globaldata):
                    globaldata[index] = [
                        nbhpt if x == str(main_point) else x for x in individualitem
                    ]
                return globaldata
    return globaldata
