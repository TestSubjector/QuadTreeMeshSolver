...
def getInteriorConditionValueofXPos(index, globaldata, hashtable):
    ...
    _, _, _, _, mypoints = deltaNeighbourCalculation(
        nbhs, getPoint(index, globaldata), True, False)
...
def getInteriorConditionValueofXNeg(index, globaldata, hashtable):
    nbhs = getNeighbours(index, globaldata)
    _, _, _, _, mypoints = deltaNeighbourCalculation(
        nbhs, getPoint(index, globaldata), True, True)
    return conditionValueForSetOfPoints(index, globaldata, mypoints)
...
def getInteriorConditionValueofYPos(index, globaldata, hashtable):
    nbhs = getNeighbours(index, globaldata)
    _, _, _, _, mypoints = deltaNeighbourCalculation(
        nbhs, getPoint(index, globaldata), False, False)
    return conditionValueForSetOfPoints(index, globaldata, mypoints)
...
def getInteriorConditionValueofYNeg(index, globaldata, hashtable):
    nbhs = getNeighbours(index, globaldata)
    _, _, _, _, mypoints = deltaNeighbourCalculation(
        nbhs, getPoint(index, globaldata), False, True)
    return conditionValueForSetOfPoints(index, globaldata, mypoints)
...
def conditionValueFixForXPos(
        index, globaldata, hashtable, threshold, wallpoints, control, flag):
    if(flag == 0):
        initialConditionValue = getInteriorConditionValueofXPos(
            index, globaldata, hashtable)
    else:
    ...
    if(initialConditionValue > threshold):
        ...
        if(len(newdxpospoints) == 0):
            writeLog(["We tried finding neighbour of neighbours but none satisfy the delta xpos condition for", getPoint(
                index, globaldata)])
        else:
            finalList = []
            nothresList = []
            for newitm in newdxpospoints:
                if(not isNonAeroDynamic(index, newitm, globaldata, wallpoints)):
                    ...
                    conditionVal = conditionValueForSetOfPoints(
                        index, globaldata, tempnbh)
            ...
            if(len(finalList) == 0 or finalList is None):
                writeLog(["We tried finding points to reduce threshold value to",
                          threshold, "but couldn't find any for index", index])
                writeLog(["It's condition value for dx pos is",
                          initialConditionValue])
                if(len(nothresList) != 0):
                    writeLog(
                        ["The least we can reduce it to is", nothresList[0][1]])
                    if(float(nothresList[0][1]) < initialConditionValue):
                        ...
                        if(flag == 0):
                            ...
                            initialConditionValue = getWeightedInteriorConditionValueofXPos(
                                index, globaldata, hashtable)
                        writeLog(
                            ["We will be running again to reduce further"])
                        if(control <= 0):
                            conditionValueFixForXPos(
                                index, globaldata, hashtable, threshold, wallpoints, control + 1, flag)
                    else:
                        writeLog(
                            ["We don't want to worsen the condition value so we are not gonna do anything else"])
                else:
                    writeLog(
                        ["We couldn't find a single point to reduce it to."])
            else:
...
def conditionValueFixForXNeg(
        index, globaldata, hashtable, threshold, wallpoints, control, flag):
    if(flag == 0):
        initialConditionValue = getInteriorConditionValueofXNeg(
            index, globaldata, hashtable)
    else:
    ...
    if(initialConditionValue > threshold):
        ...
        if(len(newdxpospoints) == 0):
            writeLog(["We tried finding neighbour of neighbours but none satisfy the delta xneg condition for", getPoint(
                index, globaldata)])
        else:
            finalList = []
            nothresList = []
            ...
            if(len(finalList) == 0 or finalList is None):
                writeLog(["We tried finding points to reduce threshold value to",
                          threshold, "but couldn't find any for index", index])
                writeLog(["It's condition value for dx neg is",
                          initialConditionValue])
...
def conditionValueFixForYPos(
        index, globaldata, hashtable, threshold, wallpoints, control, flag):
    if(flag == 0):
        initialConditionValue = getInteriorConditionValueofYPos(
            index, globaldata, hashtable)
    else:
    ...
    if(initialConditionValue > threshold):
        ...
        if(len(newdxpospoints) == 0):
            writeLog(["We tried finding neighbour of neighbours but none satisfy the delta ypos condition for", getPoint(
                index, globaldata)])
        else:
            finalList = []
            nothresList = []
            ...
            if(len(finalList) == 0 or finalList is None):
                writeLog(["We tried finding points to reduce threshold value to",
                          threshold, "but couldn't find any for index", index])
                writeLog(["It's condition value for dy pos is",
                          initialConditionValue])
                nothresList.sort(key=lambda x: x[1])
                if(len(nothresList) != 0):
                    writeLog(
...
def conditionValueFixForYNeg(
        index, globaldata, hashtable, threshold, wallpoints, control, flag):
    if(flag == 0):
        initialConditionValue = getInteriorConditionValueofYNeg(
            index, globaldata, hashtable)
    else:
    ...
    if(initialConditionValue > threshold):
        ...
        if(len(newdxpospoints) == 0):
            writeLog(["We tried finding neighbour of neighbours but none satisfy the delta yneg condition for", getPoint(
                index, globaldata)])
        else:
            finalList = []
            nothresList = []
            ...
            if(len(finalList) == 0 or finalList is None):
                writeLog(["We tried finding points to reduce threshold value to",
                          threshold, "but couldn't find any for index", index])
                writeLog(["It's condition value for dy neg is",
                          initialConditionValue])
                nothresList.sort(key=lambda x: x[1])
                if(len(nothresList) != 0):
                    writeLog(
...
def printPosDeltaConditions(index, globaldata, hashtable, threshold):
    initialConditionValueXPos = getInteriorConditionValueofXPos(
        index, globaldata, hashtable)
    dSPointXPos = getDXPosPoints(index, globaldata, hashtable)
    initialConditionValueXNeg = getInteriorConditionValueofXNeg(
        index, globaldata, hashtable)
    dSPointXNeg = getDXNegPoints(index, globaldata, hashtable)
    initialConditionValueYPos = getInteriorConditionValueofYPos(
        index, globaldata, hashtable)
    dSPointYPos = getDYPosPoints(index, globaldata, hashtable)
    initialConditionValueYNeg = getInteriorConditionValueofYNeg(
        index, globaldata, hashtable)
    ...
    if(initialConditionValueXNeg > threshold or initialConditionValueXPos > threshold or initialConditionValueYPos > threshold or initialConditionValueYNeg > threshold):
        # print(index,len(dSPointXPos),initialConditionValueXPos,len(dSPointXNeg),initialConditionValueXNeg,len(dSPointYPos),initialConditionValueYPos,len(dSPointYNeg),initialConditionValueYNeg)
        writeLog([index, len(dSPointXPos), initialConditionValueXPos, len(dSPointXNeg), initialConditionValueXNeg, len(
            dSPointYPos), initialConditionValueYPos, len(dSPointYNeg), initialConditionValueYNeg])
        printPosDeltaConditions2(index, globaldata, hashtable)


def printWeighedPosDeltaConditions(index, globaldata, hashtable, threshold):
    initialConditionValueXPos = getWeightedInteriorConditionValueofXPos(
        index, globaldata, hashtable)
    dSPointXPos = getDXPosPoints(index, globaldata, hashtable)
    initialConditionValueXNeg = getWeightedInteriorConditionValueofXNeg(
        index, globaldata, hashtable)
    dSPointXNeg = getDXNegPoints(index, globaldata, hashtable)
    initialConditionValueYPos = getWeightedInteriorConditionValueofYPos(
        index, globaldata, hashtable)
    dSPointYPos = getDYPosPoints(index, globaldata, hashtable)
    initialConditionValueYNeg = getWeightedInteriorConditionValueofYNeg(
        index, globaldata, hashtable)
    dSPointYNeg = getDYNegPoints(index, globaldata, hashtable)
    # if(len(dSPointXPos) < threshold or len(dSPointXNeg) < threshold or len(dSPointYPos) < threshold or len(dSPointYNeg) > threshold):
    #     print(index,len(dSPointXPos),initialConditionValueXPos,len(dSPointXNeg),initialConditionValueXNeg,len(dSPointYPos),initialConditionValueYPos,len(dSPointYNeg),initialConditionValueYNeg)
    ...
    #     printPosDeltaConditions2(index,globaldata,hashtable)
    if(initialConditionValueXNeg > threshold or initialConditionValueXPos > threshold or initialConditionValueYPos > threshold or initialConditionValueYNeg > threshold):
        print(index, globaldata[index][1:3], len(dSPointXPos), initialConditionValueXPos, len(dSPointXNeg), initialConditionValueXNeg, len(
            dSPointYPos), initialConditionValueYPos, len(dSPointYNeg), initialConditionValueYNeg)
        writeLog([index, len(dSPointXPos), initialConditionValueXPos, len(dSPointXNeg), initialConditionValueXNeg, len(
            dSPointYPos), initialConditionValueYPos, len(dSPointYNeg), initialConditionValueYNeg])
        printPosDeltaConditions2(index, globaldata, hashtable)
...
def printPosDeltaConditions2(index, globaldata, hashtable):
    initialConditionValueXPos = getInteriorConditionValueofXPos2(
        index, globaldata, hashtable)
    dSPointXPos = getDXPosPoints(index, globaldata, hashtable)
    initialConditionValueXNeg = getInteriorConditionValueofXNeg2(
        index, globaldata, hashtable)
    dSPointXNeg = getDXNegPoints(index, globaldata, hashtable)
    initialConditionValueYPos = getInteriorConditionValueofYPos2(
        index, globaldata, hashtable)
    dSPointYPos = getDYPosPoints(index, globaldata, hashtable)
    initialConditionValueYNeg = getInteriorConditionValueofYNeg2(
        index, globaldata, hashtable)
    dSPointYNeg = getDYNegPoints(index, globaldata, hashtable)
    # print(index,len(dSPointXPos),initialConditionValueXPos,len(dSPointXNeg),initialConditionValueXNeg,len(dSPointYPos),initialConditionValueYPos,len(dSPointYNeg),initialConditionValueYNeg)
    writeLog([index, len(dSPointXPos), initialConditionValueXPos, len(dSPointXNeg), initialConditionValueXNeg, len(
...
def printPosDeltaPointConditions(index, globaldata, hashtable, threshold):
    ...
    if(initialConditionValueXPos > threshold or initialConditionValueXNeg > threshold or initialConditionValueYPos > threshold or initialConditionValueYNeg > threshold):
        print(index, len(dSPointXPos), initialConditionValueXPos, len(dSPointXNeg), initialConditionValueXNeg, len(
            dSPointYPos), initialConditionValueYPos, len(dSPointYNeg), initialConditionValueYNeg)
        writeLog([index, len(dSPointXPos), initialConditionValueXPos, len(dSPointXNeg), initialConditionValueXNeg, len(
            dSPointYPos), initialConditionValueYPos, len(dSPointYNeg), initialConditionValueYNeg])


def setPosDeltaFlags(index, globaldata, hashtable,
                     threshold, problempts, flag):
    if(flag == 0):
        initialConditionValueXPos = getInteriorConditionValueofXPos(
            index, globaldata, hashtable)
        initialConditionValueXNeg = getInteriorConditionValueofXNeg(
...
def fixPsuedoWallPoints(index, globaldata, hashtable, wallpoints, threshold):
    ...
    for wallpointset in wallpoints:
        for idx, _ in enumerate(wallpointset):
            ...
            wallline = shapely.geometry.LineString(
                [[ptax, ptay], [ptbx, ptby]])
            distance = perpendicularDistance(
                wallpointset[pta], wallpointset[ptb], main_point)
            ...
            if(distance <= threshold):
                ...
                nbhpt = getPoint(getIndexOf(
                    wallpointset[closestwallpointnbh], hashtable), globaldata)
                for index, individualitem in enumerate(globaldata):
                    globaldata[index] = [nbhpt if x == str(
                        main_point) else x for x in individualitem]
                return globaldata
    return globaldata
